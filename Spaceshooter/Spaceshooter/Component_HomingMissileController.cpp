...
void Component_HomingMissileController::Update() {
	...
	float time_step = (float)Time::delta_time * (this->movement_speed + (float)Time::delta_time);
	transform->position.XValue = (1 - time_step) * this->initial_position.XValue + time_step * player_transform->position.XValue;
	transform->position.YValue = (1 - time_step) * this->initial_position.YValue + time_step * player_transform->position.YValue;
}
