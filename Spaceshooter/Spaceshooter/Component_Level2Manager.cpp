...
void Component_Level2Manager::Update() {
	...
	if (!this->intro_activated && this->level_time > this->intro_time) {
		this->scripted_event_commander_talk.lock()->Activate("text level 2 intro");
	...
	if (!this->player_spawned && this->level_time > this->spawn_player_time) {
		...
		player->GetComponent<Component_PlayerController>()->level2manager = this->shared_from_this();
	...
	// Spawn enemy ships manager.
	if (!this->enemy_ships_spawned && this->level_time > this->start_spawning_enemy_ships_time) {
		auto enemy_ships_manager = GameObjectFactory::GetInstance().CreateGameObject(GameObjectType::EnemyShipsManager, this->GetGameObject()->GetComponentRegistry());
		enemy_ships_manager->GetComponent<Component_EnemyShipsManager>()->player_transform = this->player_controller.lock()->GetGameObject()->GetComponent<Component_Transform>();
		Scene::Instantiate(enemy_ships_manager);

		this->enemy_ships_spawned = true;
	}

	// Play cutscenes and outro.
	if (!this->cutscene1_activated && this->level_time > this->cutscene1_time) {
		this->scripted_event_commander_talk.lock()->Activate("text level 2 cutscene 1");
		this->cutscene1_activated = true;
	} else if (!this->cutscene2_activated && this->level_time > this->cutscene2_time) {
		this->scripted_event_commander_talk.lock()->Activate("text level 2 cutscene 2");
		this->cutscene2_activated = true;
	} else if (!this->cutscene3_activated && this->level_time > this->cutscene3_time) {
		this->scripted_event_commander_talk.lock()->Activate("text level 2 cutscene 3");
		this->cutscene3_activated = true;
	} else if (!this->outro_activated && this->level_time > this->outro_time) {
		this->scripted_event_commander_talk.lock()->Activate("text level 2 outro");
...
void Component_Level2Manager::LevelFailed() {
